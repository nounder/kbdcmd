...
public final class Snippets {
  private static let backspaceKeyCode: CGKeyCode = UInt16(Key.Named.delete.rawValue)
  ...
  private static func deleteCharacters(count: Int) {
    ...
    for _ in 0..<count {
      KeyboardSimulator.simulateKeyPress(keyCode: backspaceKeyCode, flags: [])
    }
  }
  ...
  public static func expandSnippet(for sequence: [KeyPress], insert text: String) {
    ...
    for ch in text {
      if let keyCode = KeyListener.stringToKeyCode(char: String(ch)) {
        KeyboardSimulator.simulateKeyPress(keyCode: keyCode, flags: [])
      }
    }
  }
}
